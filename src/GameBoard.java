...
/*
 * the class creates logics for manipulating the tiles, including its movements, as well as
 * the logics of the game, like criteria of ending the game, etc.
 */
public class GameBoard {
    ...
    public GameBoard(int x, int y){
        ...
        scoreFont = myFont.deriveFont(24F);
        this.x = x;
        this.y = y;
    ...
    private void createBoardImage(){
        ...
        for (int row = 0; row < rows; row++){
            for (int col = 0; col < cols; col++){
                ...
                g.fillRoundRect(x, y, Tile.width,
                        Tile.height, Tile.arc_width, Tile.arc_height);
            }
        }
    }
    ...
    private void spawnRandom() {
        ...
        while (notValid){
            // convert space from 1-d to 2-d
            int location = random.nextInt(rows * cols);
            int row = location / rows;
            int col = location % cols;
            Tile current = board[row][col];
            if (current == null){
                /* chance of new number to be 2 (90%), and 4 (10%).
                 * if less than 9, be 2; otherwise 4
                 */
    ...
    public int getTileX(int col) {
        // indent x position based on spacing
    ...
    public void render(Graphics2D g){
        ...
        g.drawString("Best: " + highScore,
                Game.width-DrawUtils.getMessageWidth("Best: " +
                        highScore, scoreFont, g)-20, 40);
        g.setColor(Color.BLACK);
        g.drawString("Time: " + formattedTime, 30, 90);
        g.setColor(Color.red);
        g.drawString("Fastest: " + formatTime(fastestMS),
                Game.width-DrawUtils.getMessageWidth("Fastest: " +
                        formatTime(fastestMS), scoreFont, g)-20, 90);
    }
    ...
    private void moveTiles(Direction dir) {
        ...
        if (dir == Direction.left){
            ...
            horizontalDirection = 1;
            for (int row = 0; row < rows; row++){
                for (int col = cols-1; col >= 0; col--){ // update from right to left
                    if (!canMove){
                        canMove = move(row, col, horizontalDirection, verticalDirection, dir);
                    }
                    else{
    ...
    private boolean move(int row, int col, int horizontalDirection,
        ...
        while (move){
            ...
            // checking the tile if it's null
